    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-grow">
      <div className="mb-10 text-center">
        <h1 className="text-4xl font-extrabold text-white flex items-center justify-center gap-2">
          <PlusCircle className="h-9 w-9 text-cyan-400" />
          Create a Campaign
        </h1>
      ...
      <form onSubmit={handleSubmit} className="glass-panel rounded-2xl p-8 border border-white/5 space-y-6">
        ...
        {/* Image Upload */}
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-2">Campaign Image *</label>
          <div className="relative border border-dashed border-slate-800 rounded-xl overflow-hidden bg-slate-900/40 hover:bg-slate-900/60 hover:border-indigo-500/50 transition-colors">
            <input
              type="file"
              accept="image/*"
              disabled={isFormPending}
            ...
            {imagePreview ? (
              <div className="relative h-64 w-full">
                ...
                <div className="absolute inset-0 bg-slate-950/45 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
              ...
              <div className="p-8 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm font-medium text-slate-300 mb-1">Click or drag image file here</p>
                <p className="text-xs text-slate-500">Supports PNG, JPG, GIF, SVG (Recommended: 16:9 aspect ratio)</p>
              </div>
            )}
          </div>
        </div>
        ...
        <button
          type="submit"
          disabled={isFormPending}
          className="w-full py-4 bg-cyan-500 hover:bg-cyan-500 disabled:bg-slate-800 disabled:text-slate-500 text-white font-semibold rounded-xl flex items-center justify-center gap-2 transition-all duration-200"
        >
          {isFormPending ? (
            <>
